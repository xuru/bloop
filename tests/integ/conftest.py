...
def pytest_report_header(config):
    """
    Display extra information at test start
    """
    lines = []
    localdir = config.rootdir.join('.dynamodb-local').strpath
    if not os.path.exists(localdir):
        lines.append("*" * 100)
        lines.append(f"* NOTE: Dynamodb-local doesn't exist, installing at {localdir}")
        lines.append("*" * 100)
    return lines
